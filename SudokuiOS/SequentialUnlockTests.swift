...
struct SequentialUnlockTests {
    ...
    func level1AlwaysUnlocked() async throws {
        ...
        recalculateLocks(for: &levels, hasRemovedAds: false, debugUnlock: false)
    ...
    @Test("Remove Ads IAP unlocks all levels")
    func removeAdsUnlocksAll() async throws {
        // Given: Only Level 1 solved, but user purchased Remove Ads
        var levels = createTestLevels(count: 10, solvedUpTo: 1)
        
        // When: Recalculate locks with hasRemovedAds = true
        recalculateLocks(for: &levels, hasRemovedAds: true, debugUnlock: false)
        
        // Then: All levels should be unlocked
        let allUnlocked = levels.allSatisfy { !$0.isLocked }
        #expect(allUnlocked, "All levels should be unlocked when user has removed ads")
    }
    ...
    func debugUnlockWorksCorrectly() async throws {
        ...
        recalculateLocks(for: &levels, hasRemovedAds: false, debugUnlock: true)
    ...
    private func recalculateLocks(for levels: inout [SudokuLevel], hasRemovedAds: Bool, debugUnlock: Bool) {
        // You'll need to expose this static method or create a test-friendly version
        // LevelViewModel.recalculateLocks(for: &levels, hasRemovedAds: hasRemovedAds, debugUnlock: debugUnlock)
