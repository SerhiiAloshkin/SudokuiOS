...
> ℹ️ **CLARIFIED (Sept 18, 2026):** an earlier pass of this review flagged the Classic/
> Non-Consecutive toggle exclusivity below as a possible validation bug (base row/col/box
> uniqueness silently dropped for Non-Consecutive levels). Per confirmation from the team,
> that reading was wrong: the toggle in the level builder is a **display-only convenience** —
> it picks which single primary `ruleType` badge/label to show for the level (the same kind of
> single-value simplification already described for King/Knight/Sandwich/Classic priority
> below), not a switch that removes `.classic` from the actual `[SudokuRule]` validation set.
> Classic row/col/box uniqueness is still enforced for Non-Consecutive levels regardless of
> this toggle's state — Non-Consecutive is additive on top of Classic, exactly as the variant is
> supposed to work. No code change is implied here; this note exists only so this document
> doesn't misrepresent the toggle as a rules gap. See §3.4 for where the toggle actually lives.
...
- **Known duplicate implementation risk:** `PointingPairsSolver.swift:108-162` reimplements this
  ...
  both places or they will silently diverge. See §6.3 for a consolidated view of every duplicated
...
- > ⚠️ **REVIEW NOTE:** fail-open is the riskier default here. A corrupted or badly-generated
  > parity string silently downgrades an Odd-Even level to plain Classic, and the only visible
  > sign is a mismatch between the on-screen square/circle cues and what's actually enforced.
  > Consider fail-closed (flag the level as invalid / refuse to load it) instead of silently
...
- Non-Consecutive is a constraint **added on top of** Classic, not a replacement for it —
  confirmed intended behavior. The level builder's Classic/Non-Consecutive toggle (§3.4) only
  controls which single label is displayed for the level; it does not affect which rules are
  actually enforced.
...
> ⚠️ **REVIEW NOTE:** step 3 runs exactly once, after the pruning loop has already stopped, rather
> than being folded into the up-to-10-pass loop. A contradiction that only becomes visible after
> step 3's own eliminations (a chained contradiction) would be missed on this pass. If highlighting
> ever looks "incomplete" for a full board, this is the first place to check.
...
- No brute-force unique-solution check. The only check is the advisory `HumanLogicSolver`-based
  ...
  > ⚠️ **REVIEW NOTE:** confirm this is an intentional product decision (e.g. "custom levels are
  > sandbox/puzzle-for-its-own-sake, no uniqueness guarantee required") rather than an oversight.
  > If it's intentional, it's worth stating that explicitly here so a future contributor doesn't
  > "fix" it into a save gate by mistake; if it's not intentional, community-shared unsolvable
...
- Rule-toggle exclusivity: Classic and Non-Consecutive are mutually exclusive (picking one clears
  ...
  > see §1.0. Picking Non-Consecutive here only affects which single primary `ruleType` label is
  > shown for the level; it does not remove Classic's row/col/box uniqueness enforcement.
  > Non-Consecutive levels are still fully classic Sudoku plus the extra constraint underneath,
  > regardless of how this toggle is set. No fix needed here.
...
- Synthetic negative `id` derived from `UUID.hashValue % 1000` — only 1000 distinct buckets, a
  ...
  > ⚠️ **REVIEW NOTE:** because per-level progress for custom levels is keyed by this `id` (§3.5,
  > self-hosted fields on `CustomSudokuLevel`, not `UserLevelProgress`), a collision wouldn't
  > just be a display glitch — it could plausibly mix or overwrite one custom level's saved
  > progress with another's. 1000 buckets is a small space for a hash-derived id; consider
  > keying persistence off the `UUID` itself (or a wider hash range) instead of a mod-1000
  > bucket, especially as the number of user-created levels grows.
...
4. Clear `hasCross` (⚠ NOT recorded in move history — a known gap; undoing a value placement will
   ...
   > ⚠️ **REVIEW NOTE:** low-severity but simple to fix — record this as its own `"Cross"` history
   > entry sharing the same `batchID` as the value placement, the same way note-pruning already
   > does (§4.4), so undo restores it symmetrically.
...
- **Custom levels** (no stored solution): validated mathematically via
  `SudokuValidator().validate(board:rules:)` against the level's active rule set. See §1.0/§3.4
  for the open question about whether this path reliably enforces base row/col/box uniqueness
  for Non-Consecutive-only custom levels.
...
**Note:** `MoveHistoryManager.swift` was deleted in Phase 1 cleanup (Sept 18, 2026) - it was
never wired in and had a 100-move cap that the real system doesn't have.
...
- No cap on history size in the live implementation (the 100-move cap only existed in the deleted
  `MoveHistoryManager`).
...
- > ⚠️ **REVIEW NOTE:** custom levels never count mistakes toward game-over at all (§4.2 step 6
  > only runs `if !isCustomLevel`) — there is effectively no 3-strike fail state for custom
  > levels, even though rule violations are still visually flagged as mistakes per the "Display"
  > bullet above. If "custom levels can't be lost, only solved or abandoned" is the intended
  > design, it'd be worth stating that explicitly as a product decision rather than leaving it as
  > an implicit consequence of `solutionArray` being empty for custom levels.

### 4.7 Timer

**Note:** `TimerManager.swift` was deleted in Phase 1 cleanup (Sept 18, 2026) - it was never
wired in. Real timer logic is inline here.
...
- `shouldRunTimer`: `!isPaused && !isSettingsPresented && !isRulesPresented && !isGameComplete`
  ...
  `isSolved` by ~0.5s).
  > ⚠️ **REVIEW NOTE:** `completeGame()` stops the timer immediately on solve, so this is benign
  > in the normal flow. The edge case worth checking: if the app is backgrounded/foregrounded
  > (a scene-phase transition, which also starts/stops the timer) during that ~0.5s window
  > between `isSolved` becoming true and `isGameComplete` becoming true, could the timer
  > restart for an already-solved board? Checking `isSolved` directly in `shouldRunTimer` (in
  > addition to `isGameComplete`) would close that window if it's real.
...
- `stopTimer()` calls the **debounced** `saveState()` (2s delay), not the immediate variant — a
  ...
  > ⚠️ **REVIEW NOTE:** since `stopTimer()` is called precisely on backgrounding (`.background`/
  > `.inactive`), that's exactly the moment a force-kill becomes likely (user swipes the app away
  > right after switching out). Swapping this specific call site to `saveStateImmediate()` is a
  > small, low-risk change that would close this window entirely — worth doing given how directly
  > it maps to real "I lost my progress" support reports.

### 4.8 Save / load — two channels, both inline (not via `GamePersistenceManager`)

**Note:** `GamePersistenceManager.swift` was deleted in Phase 1 cleanup (Sept 18, 2026) - it was
never wired in. Real persistence is inline here.
...
- Three separate hand-maintained copies of Knight/King offset arrays; two separate hand-maintained
  ...
  copies. See §6.3 for why this is worth prioritizing above just "nice to have."
...
## 6. Code Review Findings — Logic Consistency Pass (Sept 18, 2026)

This section consolidates a review pass looking for internal inconsistencies, Sudoku-rule
correctness issues, and quality risks across the document above. Individual findings are also
inlined as callouts next to the relevant section; this is the at-a-glance summary, ordered
roughly by priority. Most items here are "verify, then fix" rather than confirmed bug reports,
except where a note says a test already reproduces it.

> ℹ️ **Resolved during review:** an earlier pass flagged the Classic/Non-Consecutive level-builder
> toggle (§1.0, §3.4) as a possible validation gap. This was clarified as a non-issue: the toggle
> only controls which single display label is shown for a level, not which rules are actually
> enforced. Non-Consecutive levels are fully classic Sudoku plus the extra constraint underneath,
> exactly as the variant is supposed to work. No fix needed.
...
- **Custom level `id` collision risk feeds directly into progress-loss risk (§3.5).**
  ...
  theoretical. Consider widening the id space or keying off the `UUID` directly.

### 6.2 Medium priority — design decisions that should be made explicit

- **No save-time validation gate for custom levels (§3.4)** — may well be intentional (sandbox
  philosophy), but should be stated as a decision rather than left implicit, so it isn't
  "fixed" by accident later.
- **Custom levels have no 3-mistake game-over (§4.6)** — a direct consequence of
  `solutionArray` being empty for custom levels, not an explicit design choice as documented.
  Worth confirming "custom levels can only be solved or abandoned, never failed" is the actual
  intended behavior.
- **Odd-Even fails open on malformed parity data (§1.7)** — a corrupted level silently downgrades
  to Classic instead of surfacing an error. Fail-closed would be safer, though this only matters
  if malformed data can actually reach this path (bundled JSON vs. user-supplied).

### 6.3 Lower priority — code quality, hygiene, and small fixes

- **Duplicated rule-logic implementations** (§1.6, §1.8, §5): row/col/box legality exists in
  three places, Kropki math in two, Knight/King offsets in three. None are currently known to be
  inconsistent, but every one of them is a place a future rule tweak can silently diverge from
  the others. Consolidating into one shared implementation (even just for the offset arrays)
  would remove an entire class of future bugs cheaply.
- **`hasCross` isn't undo-tracked (§4.2 step 4)** — cheap fix: record it as a `"Cross"` history
  entry on the same `batchID` as the value placement, consistent with how note-pruning already
  works.
- **Debounced (not immediate) save on backgrounding (§4.7)** — `stopTimer()`'s debounced
  `saveState()` creates a real, if narrow, data-loss window exactly at the moment
  (backgrounding) when a force-kill is most likely. Switching this one call site to
  `saveStateImmediate()` is low-risk and directly addresses "lost progress" reports.
- **`shouldRunTimer` doesn't check `isSolved` directly (§4.7)** — likely benign given
  `completeGame()`'s immediate `stopTimer()` call, but worth confirming the ~0.5s
  `isSolved`→`isGameComplete` window can't let the timer restart via a scene-phase transition.
- **Empty Box Contradiction Check runs once, not iteratively (§2.2)** — a chained contradiction
  that only appears after this check's own eliminations would be missed. No known regression
  test exists for this calculator's output either, so a behavior change here wouldn't be caught
  automatically.
- **Non-compiling, stale, or misdirected tests (§1.11, §3.2)** — `SudokuEngineTests.swift` /
  `SudokuRulesTests.swift` reference a nonexistent API and wrong constructor signatures;
  `testRemoveAdsUnlock` writes to the wrong UserDefaults key. These create a false sense of test
  coverage and should either be fixed or deleted (see §5's Phase 2 cleanup candidate).
