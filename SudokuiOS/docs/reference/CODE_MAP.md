...
| Adding/changing any user-facing string, or anything about localization | `Localizable.xcstrings` (project root) + `CLAUDE.md` → "Localization" section | Read the CLAUDE.md section first — it covers the `LocalizedStringKey` vs. `String(localized:)` decision, format-specifier keys for interpolated strings, and a real string-equality-as-control-flow bug it fixed |
